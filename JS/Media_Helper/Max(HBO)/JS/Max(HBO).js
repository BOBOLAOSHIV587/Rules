/*
@author: liunice
@decription: Max(HBO) iOS 外挂字幕和强制1080p插件
@created: 2023-05-31
@updated: 2023-05-31
*/
...
项目主页: https://github.com/liunice/MaxHelper
TG官方群: https://t.me/+W6aJJ-p9Ir1hNmY1

QuanX用法：
hostname = default.prd.api.discomax.com, cf.prod.media.h264.io, cf.prd.media.h264.io
...
(async () => {
    const $ = Env("max_helper.js")
    const SCRIPT_NAME = 'MaxHelper'
    const SUBTITLES_DIR = 'Subtitles'
    const FN_SUB_SYNCER_DB = 'sub_syncer.db'
    const PLATFORM_NAME = 'max'

    if (/\/cms\/routes\/video\/watch\/([0-9a-z\-]+)\/.*?/.test($request.url)) {
        const root = JSON.parse($response.body)

        const episode_id = /\/cms\/routes\/video\/watch\/([0-9a-z\-]+)\/.*?/.exec($request.url)[1]
        $.log('playing episode:', episode_id)
        
        // get show id
        for (const item of root.included) {
            if (item.id == episode_id) {
                const show_id = item.relationships.show.data.id
                checkPlayingEpisode(root, show_id)
                break
            }
        }
    ...
    else if (/\/playback-orchestrator\/any\/playback-orchestrator\/v\d+\/playbackInfo$/.test($request.url)) {
        const root = JSON.parse($response.body)

        // remove promo and Re-cap videos
        root.videos = root.videos.filter(v => v.type == 'main')
        const main = root.videos[0]
        // fix offset
        main.start = 0
        // remove `Up Next` and `skip Intro` buttons
        delete main['annotations']

        // save manifest for sub syncer
        if (getSubtitleConfig('subsyncer.enabled') == 'true') {
            writeSubSyncerDB(root.manifest.url)
        }
    ...
    else if (/\/r\/hls\.m3u8\?/.test($request.url)) {
    ...
    else if (/\/r\/hlsMedia\.m3u8\?r\.host=.*?_t_t\d+\.m3u8&/.test($request.url)) {
...
#EXTINF:36000.000,
https://cf.prod.media.h264.io/subtitles/dummy.vtt
#EXT-X-ENDLIST
`
            $.done({ body: body })
        }
    ...
    else if (/\/r\/hlsMedia\.m3u8\?r\.host=.*?_(a|v)\d+\.m3u8&/.test($request.url)) {
        if (checkSubtitleExists()) {
    ...
    function checkPlayingEpisode(root, show_id) {
        let seriesName
        let seasonNo
        let episodeNo
        for (const item of root.included) {
            ...
            if (item.attributes.showType == 'SERIES' && item.attributes.alternateId == show_id) {
                seriesName = item.attributes.name
            }
            else if (item.attributes.episodeNumber) {
                const season = item.attributes.seasonNumber || 1
                seasonNo = season.toString().padStart(2, '0')
                episodeNo = item.attributes.episodeNumber.toString().padStart(2, '0')
            }
        }

        if (seriesName && seasonNo) {
            $.setdata(seriesName, `series_name@${SCRIPT_NAME}`)
            $.setdata(seasonNo, `season_no@${SCRIPT_NAME}`)
            $.setdata(episodeNo, `ep_no@${SCRIPT_NAME}`)
            notify(SCRIPT_NAME, '正在播放剧集', `[${seriesName}] S${seasonNo}E${episodeNo}`)
        }
        else {
            clearPlaying()
        }
